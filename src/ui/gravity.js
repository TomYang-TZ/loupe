...
const Gravity = (() => {
  ...
  function getNodeColor(node, theme) {
    ...
    if (node.lastAction === "edit") return dark ? "#4ade80" : "#16a34a";
    if (node.lastAction === "exec") return dark ? "#f97316" : "#c2410c";
    if (node.lastAction === "read") return dark ? "#3b82f6" : "#2563eb";
    return dark ? "#8b8b96" : "#64748b";
  }
  ...
  function render() {
    ...
    for (const node of nodes.values()) {
      ...
      // Glow
      if (isGlowing && !dimmed) {
        const gi = 1 - (age / GLOW_DURATION);
        const rgb = hexToRgb(baseColor);
        if (rgb) {
          ctx.beginPath();
          ctx.arc(fx, fy, r + 6, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(${rgb.r},${rgb.g},${rgb.b},${gi * 0.2})`;
          ctx.fill();
        }
      }
  ...
  function initInteraction() {
    ...
    canvas.addEventListener("mouseleave", () => { fisheyeActive = false; hoveredNode = null; });
  ...
  return { init, addEntry, addEntries, destroy, reset, getTooltip, zoom, deselect, getNodes, getEdges };
})();
